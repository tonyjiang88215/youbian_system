/**
 * 管理章节单元页面数据处理中心
 * @Author By TonyJiang
**/

var origin_search = 'origin_search';

//数据查询和提交对象
dataQueryObject = {
	queryTreeData : function(){
		
		var book_id = TJDataCenter.get('book_id');
		
		$.ajax({
			'url' : TJDataCenter.get('webroot')+'/manage_system/api/base/unit_chapter_api.php?action=tree_data',
			'type' : 'GET',
			'dataType' : 'jsonp',
			'data' : {
				subject : TJDataCenter.get(origin_search+'_subject'),
				book : book_id
			},
			success : function(data){
				if(data){
					var e = new TJEvent.EventObject('query_tree_success');
					e.data = data;
					e.data.group = 'query';
					TJEvent.dispatch(e);
					
				}else{
					var e = new TJEvent.EventObject('query_tree_failed');
					e.data.source = 'unit_chapter_handler.js queryTreeData()';
					TJEvent.dispatch(e);
				}
			},
			error : function(){
				var e = new TJEvent.EventObject('query_tree_failed');
				e.data.source = 'unit_chapter_handler.js queryTreeData()';
				TJEvent.dispatch(e);
			}
		});
		
	},
	
	queryBookData : function(){
		
		var treeData = [];

		var e1 = new TJEvent.EventObject('refresh_tree');
		e1.data.treeData = treeData;
		e1.data.classSelector = '.treeClass';
		e1.data.flag = true;
		TJEvent.dispatch(e1);
		
		
		$.ajax({
			'url' : TJDataCenter.get('webroot')+'/manage_system/api/base/unit_chapter_api.php?action=book_data',
			'type' : 'GET',
			'dataType' : 'jsonp',
			'data' : {
				subject : TJDataCenter.get(origin_search+'_subject'),
				section : TJDataCenter.get(origin_search+'_section'),
				publisher : TJDataCenter.get(origin_search+'_publisher')
			},
			success : function(data){
				if(data){
					var e = new TJEvent.EventObject('query_book_success');
					e.data = data;
					TJEvent.dispatch(e);
					
				}else{
					var e = new TJEvent.EventObject('ajax_error');
					e.data.source = 'unit_chapter_handler.js queryBookData()';
					TJEvent.dispatch(e);
				}
			},
			error : function(){
				var e = new TJEvent.EventObject('ajax_error');
				e.data.source = 'unit_chapter_handler.js queryBookData()';
				TJEvent.dispatch(e);
			}
		});
	}
	
};

dataSubmitObject = {
	
	insertUnit : function(newData){
		$.ajax({
			'url' : TJDataCenter.get('webroot')+'/manage_system/api/base/unit_chapter_api.php?action=insert_unit',
			'type' : 'POST',
			'dataType' : 'jsonp',
			'data' : {
				new_data : newData
			},
			success : function(data){
				if(data){
					var e = new TJEvent.EventObject('insert_unit_success');
					TJEvent.dispatch(e);
				}else{
					var e = new TJEvent.EventObject('insert_unit_failed');
					TJEvent.dispatch(e);
				}
			},
			error : function(){
				var e = new TJEvent.EventObject('insert_unit_failed');
				TJEvent.dispatch(e);
			}
		});
	},
	
	insertChapter : function(newData){
		$.ajax({
			'url' : TJDataCenter.get('webroot')+'/manage_system/api/base/unit_chapter_api.php?action=insert_chapter',
			'type' : 'POST',
			'dataType' : 'jsonp',
			'data' : {
				new_data : newData
			},
			success : function(data){
				if(data){
					var e = new TJEvent.EventObject('insert_chapter_success');
					TJEvent.dispatch(e);
				}else{
					var e = new TJEvent.EventObject('insert_chapter_failed');
					TJEvent.dispatch(e);
				}
			},
			error : function(){
				var e = new TJEvent.EventObject('insert_chapter_failed');
				TJEvent.dispatch(e);
			}
		});
	},
	
	removeUnit : function(id , oldname){
		$.ajax({
			'url' : TJDataCenter.get('webroot')+'/manage_system/api/base/unit_chapter_api.php?action=remove_unit',
			'type' : 'POST',
			'dataType' : 'jsonp',
			'data' : {
				id : id, 
				subject : TJDataCenter.get(origin_search+'_subject'),
				oldname : oldname
			},
			success : function(data){
				if(data){
					var e = new TJEvent.EventObject('remove_unit_success');
					TJEvent.dispatch(e);
				}else{
					var e = new TJEvent.EventObject('remove_unit_failed');
					TJEvent.dispatch(e);
				}
			},
			error : function(){
				var e = new TJEvent.EventObject('remove_unit_failed');
				TJEvent.dispatch(e);
			}
		});
	},
	
	removeChapter : function(id , oldname){
		$.ajax({
			'url' : TJDataCenter.get('webroot')+'/manage_system/api/base/unit_chapter_api.php?action=remove_chapter',
			'type' : 'POST',
			'dataType' : 'jsonp',
			'data' : {
				id : id, 
				subject : TJDataCenter.get(origin_search+'_subject'),
				oldname : oldname
			},
			success : function(data){
				if(data){
					var e = new TJEvent.EventObject('remove_chapter_success');
					TJEvent.dispatch(e);
				}else{
					var e = new TJEvent.EventObject('remove_chapter_failed');
					TJEvent.dispatch(e);
				}
			},
			error : function(){
				var e = new TJEvent.EventObject('remove_chapter_failed');
				TJEvent.dispatch(e);
			}
		});
	},
	
	modifyElement : function(type , id , name , oldname){
		$.ajax({
			'url' : TJDataCenter.get('webroot')+'/manage_system/api/base/unit_chapter_api.php?action=modify',
			'type' : 'POST',
			'dataType' : 'jsonp',
			'data' : {
				id : id, 
				subject : TJDataCenter.get(origin_search+'_subject'),
				name : name,
				oldname : oldname,
				type : type
			},
			success : function(data){
				if(data){
					var e = new TJEvent.EventObject('modify_element_success');
					TJEvent.dispatch(e);
				}else{
					var e = new TJEvent.EventObject('modify_element_failed');
					TJEvent.dispatch(e);
				}
			},
			error : function(){
				var e = new TJEvent.EventObject('modify_element_failed');
				TJEvent.dispatch(e);
			}
		});
	},
	
	moveUpElement : function(id , type , oldname){
		$.ajax({
			'url' : TJDataCenter.get('webroot')+'/manage_system/api/base/unit_chapter_api.php?action=move_up',
			'type' : 'POST',
			'dataType' : 'jsonp',
			'data' : {
				id : id, 
				subject : TJDataCenter.get(origin_search+'_subject'),
				type : type , 
				oldname : oldname
			},
			success : function(data){
				if(data){
					var e = new TJEvent.EventObject('move_up_success');
					TJEvent.dispatch(e);
				}else{
					var e = new TJEvent.EventObject('move_up_failed');
					TJEvent.dispatch(e);
				}
			},
			error : function(){
				var e = new TJEvent.EventObject('move_up_failed');
				TJEvent.dispatch(e);
			}
		});
	},
	
	moveDownElement : function(id ,type , oldname){
		$.ajax({
			'url' : TJDataCenter.get('webroot')+'/manage_system/api/base/unit_chapter_api.php?action=move_down',
			'type' : 'POST',
			'dataType' : 'jsonp',
			'data' : {
				id : id, 
				subject : TJDataCenter.get(origin_search+'_subject'),
				type : type ,
				oldname : oldname
			},
			success : function(data){
				if(data){
					var e = new TJEvent.EventObject('move_down_success');
					TJEvent.dispatch(e);
				}else{
					var e = new TJEvent.EventObject('move_down_failed');
					TJEvent.dispatch(e);
				}
			},
			error : function(){
				var e = new TJEvent.EventObject('move_down_failed');
				TJEvent.dispatch(e);
			}
		});
	},
	
	dragChapter : function(info , oldname){
		$.ajax({
			'url' : TJDataCenter.get('webroot')+'/manage_system/api/base/unit_chapter_api.php?action=drag',
			'type' : 'POST',
			'dataType' : 'jsonp',
			'data' : {
				chapter : info , 
				subject : TJDataCenter.get(origin_search+'_subject'),
				oldname : oldname
			},
			success : function(data){
				if(data){
					var e = new TJEvent.EventObject('drag_chapter_success');
					TJEvent.dispatch(e);
				}else{
					var e = new TJEvent.EventObject('drag_chapter_failed');
					TJEvent.dispatch(e);
				}
			},
			error : function(){
				var e = new TJEvent.EventObject('drag_chapter_failed');
				TJEvent.dispatch(e);
			}
		});
	},
	
	clearData : function(){
		$.ajax({
			'url' : TJDataCenter.get('webroot')+'/manage_system/api/base/unit_chapter_api.php?action=clear_data',
			'type' : 'POST',
			'dataType' : 'jsonp',
			'data' : {
				subject : TJDataCenter.get(origin_search+'_subject'),
				book : TJDataCenter.get('book_id')
			},
			success : function(data){
				if(data){
					var e = new TJEvent.EventObject('clear_data_success');
					TJEvent.dispatch(e);
				}else{
					var e = new TJEvent.EventObject('clear_data_failed');
					TJEvent.dispatch(e);
				}
			},
			error : function(){
				var e = new TJEvent.EventObject('clear_data_failed');
				TJEvent.dispatch(e);
			}
		});
	}
};

TJEvent.addListener('publisher_change' , function(e){
	
	if(e.data.group != 'origin_search'){
		return;
	}
	
	//选择了学科，学段和出版社后，才能获取数据
	if(TJDataCenter.get(origin_search+'_subject') != 0 && TJDataCenter.get(origin_search+'_section') != 0 && TJDataCenter.get(origin_search+'_publisher') != 0){
		dataQueryObject.queryBookData();
	}else{
		
		$('.action_panel').addClass('action_disabled');
		
		var ev = new TJEvent.EventObject('query_book_success');
		ev.data.length = 0;
		TJEvent.dispatch(ev);
		

		var treeEv = new TJEvent.EventObject('refresh_tree');
		treeEv.data.treeData = [];
		treeEv.data.classSelector = '.treeClass';
		treeEv.data.flag = true;
		TJEvent.dispatch(treeEv);
		
	}
});


TJEvent.addListener('select_book_change' , function(){
	
	$('.action_insert_unit , .action_setin  , .action_clear').removeClass('action_disabled');
	
	$('.action_insert_chapter , .action_move_up , .action_move_down , .action_delete').addClass('action_disabled');
	
	var book_id = TJDataCenter.get('book_id');
	if(book_id != 0){
		dataQueryObject.queryTreeData();
	}
	
});

TJEvent.addListener('tree_element_selected' , function(e){
	var current_element = TJDataCenter.get('origin_tree_selected_tree_element');
	
	//如果是单元节点
	if(current_element.attr('type') == 'unit'){
		TJDataCenter.set('current_unit_id' , current_element.attr('elementid'));
		$('.action_insert_chapter').removeClass('action_disabled');
		
		if(current_element.closest('li').next().find('li.link').length == 0){
			$('.action_delete').removeClass('action_disabled');
		}else{
			$('.action_delete').addClass('action_disabled');
		}
		
	//如果是章节节点	
	}else{
		$('.action_insert_chapter').addClass('action_disabled');
		$('.action_delete').removeClass('action_disabled');
	}
	
	$('.action_move_up , .action_move_down').removeClass('action_disabled');
});

TJEvent.addListener('query_book_success' , function(e){
	
	$('.book_wrapper').html('');
	
	var bookHTML = '';
	
	for(var i = 0 ; i < e.data.length ; i++){
		bookHTML += '<div class="book_list" book_id="'+e.data[i].id+'" grade_id="'+e.data[i].grade_id+'" publisher_id="'+e.data[i].publisher_id+'" >'+e.data[i].book_name+'</div>';
	} 
	
	if(e.data.length == 0){
		bookHTML = '<div style="color:#C1C1C1;">暂无</div>';
	}
	
//	console.log($('.select_book'));
	$('.book_wrapper').append(bookHTML);
	
	TJDataCenter.set('book_id' , 0);
	
});

TJEvent.addListener('query_tree_success',function(e){
	
	if(e.data.group != 'query'){
		return;
	}
	
	var data = e.data;
	
	var treeData = [];
	
	if(data.unit.length == 0 && data.chapter.length == 0){
		
//		treeData.push({id:-1 , name:'暂无数据' , children : [] , attributes : {level : 1 , parent_id : 0 , sort_id : 0}});
		
	}else{
		
		for(var i = 0 ; i < data.unit.length ; i++){
			treeData.push({
				id : data.unit[i].id , 
				name : data.unit[i].unit_name , 
				children : [] , 
				attributes : {
					unit_index : data.unit[i].unit_index , 
					type : 'unit'
				} 
			});
		}
		
		for(var i = 0 ; i < data.chapter.length ; i++){
			for(var j = 0 ; j < treeData.length ; j++){
				if(data.chapter[i].unit_id == treeData[j].id){
					treeData[j].children.push({
						id : data.chapter[i].id , 
						name : data.chapter[i].chapter_name , 
						children : [],
						attributes : {
							chapter_code : data.chapter[i].chapter_code , 
							chapter_index : data.chapter[i].chapter_index , 
							type : 'chapter'
						}
					});
				}
			}
		}
		
		
	}
	
	var lastElement = TJDataCenter.get('origin_tree_selected_tree_element');
	if(lastElement){
		TJDataCenter.set('last_tree_element_id' , lastElement.attr('elementid'));
	}else{
		TJDataCenter.set('last_tree_element_id' , 0);
	}
	
	var e1 = new TJEvent.EventObject('refresh_tree');
	e1.data.treeData = treeData;
	e1.data.classSelector = '.treeClass';
	e1.data.flag = true;
	TJEvent.dispatch(e1);
});

TJEvent.addListener('query_tree_failed' , function(){
	
	if(e.data.group != 'query'){
		return;
	}
	
	var treeData = [];
//	treeData.push({id:-1 , name:'暂无数据' , children : [] , attributes : {level : 1 , parent_id : 0 , sort_id : 0}});
	var e = new TJEvent.EventObject('refresh_tree');
	e.data.classSelector = '.treeClass';
	e.data.treeData = treeData;
	
	TJEvent.dispatch(e);
});

TJEvent.addListener('refresh_tree_success' , function(e){ 
	var last_id = TJDataCenter.get('last_tree_element_id');
	if(last_id){
		setTimeout(function(){
			var element = $('li[elementid='+last_id+']');
			if(element.length > 0){
				$('li[elementid='+last_id+']').find('a').click().focus();
			}else{
				TJDataCenter.set('last_tree_element_id' , 0);
				TJDataCenter.set('origin_tree_selected_tree_element' , 0);
			}
		},100);
	}
});


TJEvent.addListener('tree_element_dbclick' , function(e){
	
	var ev = new TJEvent.EventObject('edit_name_show');
	ev.data.name = $(e.data.htmlObject).text();
	TJEvent.dispatch(ev);
	
});

//编辑保存时处理
TJEvent.addListener('edit_name_submit' , function(e){
	
	var id = TJDataCenter.get('origin_tree_dbclick_tree_element').attr('elementid');
	var type = TJDataCenter.get('origin_tree_dbclick_tree_element').attr('type');
	var oldname = TJDataCenter.get('origin_tree_dbclick_tree_element').attr('title');
	
	dataSubmitObject.modifyElement(type ,  id , e.data.name , oldname);

});


TJEvent.addListener('insert_unit_submit' , function(e){
	
	var data = {
//		pub_id : TJDataCenter.get('pub_id'),
		book_id : TJDataCenter.get('book_id'),
//		book_code : TJDataCenter.get('book_code'),
		unit_name : e.data.name,
		unit_index : $('#treeNavDIV[group=origin_tree] a[type=unit]:last').length > 0 ? parseInt($('#treeNavDIV a[type=unit]:last').attr('unit_index'))+1 : 1,
		subject_id : TJDataCenter.get(origin_search+'_subject'),
//		grade_id : TJDataCenter.get('grade_id'),
//		section_id : TJDataCenter.get('section'),
//		source : '',
//		version_id : TJDataCenter.get('curriculumn_version')
	};
	
//	data.unit_code = data.unit_index.pad(2);
	
	dataSubmitObject.insertUnit(data);	
	
});

TJEvent.addListener('modify_element_success' , function(){
	dataQueryObject.queryTreeData();
	
	var ev = new TJEvent.EventObject('edit_finish');
	TJEvent.dispatch(ev);
	
});

TJEvent.addListener('insert_unit_success' , function(){
	
	dataQueryObject.queryTreeData();
	
	var ev = new TJEvent.EventObject('edit_finish');
	TJEvent.dispatch(ev);
	
});

TJEvent.addListener('insert_chapter_submit' , function(e){
	var data = {
		chapter_name : e.data.name,
		chapter_index : TJDataCenter.get('origin_tree_selected_tree_element').closest('li').next().find('a:last').length > 0 ? 
			parseInt(TJDataCenter.get('origin_tree_selected_tree_element').closest('li').next().find('a:last').attr('chapter_index'))+1 : 1,
		book_id : TJDataCenter.get('book_id'),
		unit_id : TJDataCenter.get('current_unit_id'),
//		unit_code : TJDataCenter.get('current_unit_code'),
//		pub_id : TJDataCenter.get('pub_id'),
//		book_code : TJDataCenter.get('book_code'),
		subject_id : TJDataCenter.get(origin_search+'_subject'),
//		grade_id : TJDataCenter.get('grade_id'),
//		section_id : TJDataCenter.get('section'),
//		source : '',
//		version_id : TJDataCenter.get('curriculumn_version')
	};
	
//	data.chapter_code = data.chapter_index.pad(2);
	
	dataSubmitObject.insertChapter(data);	
	
});

TJEvent.addListener('insert_chapter_success', function(){
	dataQueryObject.queryTreeData();
	
	var ev = new TJEvent.EventObject('edit_finish');
	TJEvent.dispatch(ev);
});

TJEvent.addListener('remove_unit_success' , function(){
	
	dataQueryObject.queryTreeData();
	
});

TJEvent.addListener('remove_chapter_success' , function(){
	
	dataQueryObject.queryTreeData();
	
});

TJEvent.addListener('move_up_success' , function(){
	
	dataQueryObject.queryTreeData();
	
});

TJEvent.addListener('move_down_success' , function(){
	
	dataQueryObject.queryTreeData();
	
});

//拖拽修改位置时处理
TJEvent.addListener('drop_tree_element' , function(e){
	
	if($(e.data.object).find('a').attr('type') != 'chapter'){
		alert('只能拖拽章节');
		return;
	}
	
	if($(e.data.target).find('a').attr('type') != 'unit'){
		alert('请将章节拖拽到单元上');
		return;
	}
	
	var info = {
		id : $(e.data.object).attr('elementid'),
		unit_id : $(e.data.target).attr('elementid'),
//		unit_code : $(e.data.target).attr('unit_code'),
		chapter_index : $(e.data.target).next().children('.child_ul').children('li[elementid]').length + 1 
	};
//	info.chapter_code = info.chapter_index.pad(2);
console.log(info);
	
	dataSubmitObject.dragChapter(info , $(e.data.object).find('a').attr('title'));
	
});

TJEvent.addListener('drag_chapter_success' , function(e){
	dataQueryObject.queryTreeData();
});

TJEvent.addListener('clear_data_success' , function(e){
	alert('数据清除成功');
	
	var ev = new TJEvent.EventObject('select_book_change');
	TJEvent.dispatch(ev);
});

$(document).ready(function(){
	
	$('.book_wrapper').delegate('.book_list' , 'click' , function(){
		
		var book = $(this);
		
		TJDataCenter.set('book_id' , book.attr('book_id'));
		TJDataCenter.set('publisher_id' , book.attr('publisher_id'));
		TJDataCenter.set('grade_id' , book.attr('grade_id'));
		
		$('.book_list_selected').removeClass('book_list_selected');
		
		$(this).addClass('book_list_selected');
		
		var e = new TJEvent.EventObject('select_book_change');
		TJEvent.dispatch(e);
	});
	
	$('.action_insert_unit').click(function(){
		if($(this).hasClass('action_disabled')){
			return;
		}
		var e = new TJEvent.EventObject('insert_unit_show');
		TJEvent.dispatch(e);
	});
	
	$('.action_insert_chapter').click(function(){
		if($(this).hasClass('action_disabled')){
			return;
		}
		var e = new TJEvent.EventObject('insert_chapter_show');
		TJEvent.dispatch(e);
	});
	
	$('.action_delete').click(function(){
		if($(this).hasClass('action_disabled')){
			return;
		}
		var selectElement = TJDataCenter.get('origin_tree_selected_tree_element');
		if(!selectElement){
			alert('请选择单元或章节');
			return;
		}else{
			
			if(confirm('确认删除“'+selectElement.text()+'”吗?此操作不可恢复')){
				
				if(selectElement.attr('type') == 'unit'){
					dataSubmitObject.removeUnit(selectElement.attr('elementid') , selectElement.attr('title'));
				}else if(selectElement.attr('type') == 'chapter'){
					dataSubmitObject.removeChapter(selectElement.attr('elementid') , selectElement.attr('title'));
				}
			}
			
		}
	});
	
	//节点上移
	$('.action_move_up').click(function(){
		if($(this).hasClass('action_disabled')){
			return;
		}
		if(TJTree.checkFirst()){
			alert('已经到顶啦');
			return;
		}else{
			var selectElement = TJDataCenter.get('origin_tree_selected_tree_element');
			dataSubmitObject.moveUpElement(selectElement.attr('elementid') , selectElement.attr('type') , selectElement.attr('title'));
		}
	});
	
	//节点下移
	$('.action_move_down').click(function(){
		if($(this).hasClass('action_disabled')){
			return;
		}
		if(TJTree.checkLast()){
			alert('已经到底啦');
			return;
		}else{
			var selectElement = TJDataCenter.get('origin_tree_selected_tree_element');
			dataSubmitObject.moveDownElement(selectElement.attr('elementid') , selectElement.attr('type') , selectElement.attr('title'));
		}
	});
	
	$('.action_insert_book').click(function(){
		if($(this).hasClass('action_disabled')){
			return;
		}
		
		var e = new TJEvent.EventObject('popup_show');
		e.data.name = 'add_book';
		TJEvent.dispatch(e);
		
	});
	
	$('.action_setin').click(function(){
		if($(this).hasClass('action_disabled')){
			return;
		}
		
		if($('.origin_tree .link[elementid!=-1] , .origin_tree .parent').length > 0){
			alert('请先清空数据，在进行导入');
			return;
		}
		
		var e = new TJEvent.EventObject('popup_show');
		e.data.name = 'data_setin';
		TJEvent.dispatch(e);
		
	});
	
	$('.action_clear').click(function(){
		
		if($(this).hasClass('action_disabled')){
			return;
		}
		
		var book_id = TJDataCenter.get('book_id');
		
		
		if(confirm('确认清除“'+$('.book_list_selected').text()+'”的数据吗？此操作不可恢复')){
			
			dataSubmitObject.clearData();
			
		}
	});
	
});
