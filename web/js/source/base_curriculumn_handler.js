...
dataQueryObject = {
	...
	queryCurriculumn : function(){
		$.ajax({
			'url' : TJDataCenter.get('webroot')+'/manage_system/api/base/curriculumn_api.php?action=list',
			'type' : 'GET',
			'dataType' : 'jsonp',
	...
	queryCurriculumnByBook : function(bookid){
		$.ajax({
			'url' : TJDataCenter.get('webroot')+'/manage_system/api/base/curriculumn_api.php?action=list_by_bookid',
			'type' : 'GET',
			'dataType' : 'jsonp',
			'data' : {
				book : bookid
			...
			success : function(data){
				if(data){
					var e = new TJEvent.EventObject('query_curriculumn_by_book_success');
					e.data = data;
					TJEvent.dispatch(e);
	...
	queryBookData : function(){
		...
		var subject = TJDataCenter.get('first_subject');
		var section = TJDataCenter.get('first_section');
		var publisher = TJDataCenter.get('first_publisher');
		
		if(subject == 0 || section == 0 || publisher == 0){
			return;
		}
		
		$.ajax({
			'url' : TJDataCenter.get('webroot')+'/manage_system/api/base/book_api.php?action=book',
			'type' : 'GET',
			'dataType' : 'jsonp',
			'data' : {
				subject : subject,
	...
	queryTree : function(){
		...
		var subject = TJDataCenter.get('second_subject');
		var section = TJDataCenter.get('second_section');
		var version = TJDataCenter.get('second_curriculumn_version');
		var book = TJDataCenter.get('second_book');
		
		if(type == 1){
			
			if(book == 0 || version == 0 || subject == 0){
				return;
			}
			
		}else{
			
			if(subject == 0 || section ==0 || version == 0){
				return;
			}
		...
		$.ajax({
			'url' : TJDataCenter.get('webroot')+'/manage_system/api/base/curriculumn_detail_api.php?action=tree',
			'type' : 'GET',
			'dataType' : 'jsonp',
			'data' : {
				type : type,
			...
			success : function(data){
				if(data){
					TJDataCenter.set('current_tree' , data);
...
dataSubmitObject = {
	...
	activeTongbu : function(){
		$.ajax({
			'url' : TJDataCenter.get('webroot')+'/manage_system/api/base/curriculumn_api.php?action=active_tongbu',
			'type' : 'POST',
			'dataType' : 'jsonp',
			'data' : {
				version : TJDataCenter.get('current_curriculumn') 
			...
			success : function(data){
				if(data){
					var e = new TJEvent.EventObject('active_tongbu_success');
					e.data = data;
					TJEvent.dispatch(e);
	...
	activeKnowledge : function(){
		$.ajax({
			'url' : TJDataCenter.get('webroot')+'/manage_system/api/base/curriculumn_api.php?action=active_knowledge',
			'type' : 'POST',
			'dataType' : 'jsonp',
			'data' : {
				version : TJDataCenter.get('current_curriculumn') 
			...
			success : function(data){
				if(data){
					var e = new TJEvent.EventObject('active_knowledge_success');
					e.data = data;
					TJEvent.dispatch(e);
	...
	activeZhuanti : function(){
		$.ajax({
			'url' : TJDataCenter.get('webroot')+'/manage_system/api/base/curriculumn_api.php?action=active_zhuanti',
			'type' : 'POST',
			'dataType' : 'jsonp',
			'data' : {
				version : TJDataCenter.get('current_curriculumn') 
			...
			success : function(data){
				if(data){
					var e = new TJEvent.EventObject('active_zhuanti_success');
					e.data = data;
					TJEvent.dispatch(e);
...
TJEvent.addListener('query_curriculumn_success' , function(e){
	...
	$('.curriculumn_wrapper').html('');
	
	for(var i = 0 ; i < e.data.length ; i++){
		$('.curriculumn_wrapper').append("<a class='content_tab' href='javascript:void(0);' cid='"+e.data[i].id+"'  zhuanti='"+e.data[i].zhuanti+"' knowledge='"+e.data[i].knowledge+"' tongbu='"+e.data[i].tongbu+"' >"+e.data[i].name+"</a>");
	}
	
	var ev = new TJEvent.EventObject('ref_curriculumn_change');
	ev.data = e.data;
	TJEvent.dispatch(ev);
	
});

TJEvent.addListener('query_curriculumn_by_book_success' , function(e){
	
	var ev = new TJEvent.EventObject('search_bar_curriculumn_update');
	ev.data.info = e.data;
	TJEvent.dispatch(ev);
...
TJEvent.addListener('curriculumn_change' , function(e){
	...
	$('.type_wrapper a').removeClass('unactive').poshytip('disable');
	
	if(tongbu == 0){
		$('.type_wrapper a[type=1]').addClass('unactive').poshytip('enable');
	}
	
	if(knowledge == 0){
		$('.type_wrapper a[type=2]').addClass('unactive').poshytip('enable');
	}
	
	if(zhuanti == 0){
		$('.type_wrapper a[type=3]').addClass('unactive').poshytip('enable');
	}
...
TJEvent.addListener('active_tongbu_success' , function(){
	
	var curriculumn = TJDataCenter.get('current_curriculumn');
	
	TJDataCenter.set('current_curriculumn_tongbu' , 1);
	
	//先将版本中的信息更新
	$('.curriculumn_wrapper a[cid='+curriculumn+']').attr('tongbu' , 1);
	
	//将类型状态更新
	$('.type_wrapper a[type=1]').removeClass('unactive').poshytip('disable').trigger('click');
	
	$('.deny_cover').hide();
	
});

TJEvent.addListener('active_knowledge_success' , function(){
	
	var curriculumn = TJDataCenter.get('current_curriculumn');
	
	TJDataCenter.set('current_curriculumn_knowledge' , 1);
	//先将版本中的信息更新
	$('.curriculumn_wrapper a[cid='+curriculumn+']').attr('knowledge' , 1);
	
	//将类型状态更新
	$('.type_wrapper a[type=2]').removeClass('unactive').poshytip('disable').trigger('click');
	
	$('.deny_cover').hide();
	
});

TJEvent.addListener('active_zhuanti_success' , function(){
	
	var curriculumn = TJDataCenter.get('current_curriculumn');
	
	TJDataCenter.set('current_curriculumn_zhuanti' , 1);
	//先将版本中的信息更新
	$('.curriculumn_wrapper a[cid='+curriculumn+']').attr('zhuanti' , 1);
	
	//将类型状态更新
	$('.type_wrapper a[type=3]').removeClass('unactive').poshytip('disable').trigger('click');
