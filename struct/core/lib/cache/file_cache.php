<?php
/**
 * fileCache 类提供以文件系统来缓存数据的服务<本文件来自qeePHP>
 *
...
class fileCache
{
     ...
     *
	 * -  cache_dir_umask: 创建缓存目录时的标志，默认为 0700(权限)
     *
	 * -  cache_dir: 缓存目录（必须指定）
     *
	...
	function __construct(array $default_policy = null)
	{
		...
		if (empty($this->_default_policy['cache_dir']) || $this->_default_policy['cache_dir']=='' )
		{
		    $this->_default_policy['cache_dir'] = s8s_get_path('tmp')."/cachedata/";
		}
	}
	...
	protected function _path($id, array $policy)
	{
        ...
        if (empty($root_dir))
        {
            throw new Exception('cache_dir must be a directory');
        }
