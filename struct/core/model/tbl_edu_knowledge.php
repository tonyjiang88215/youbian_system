<?php
class tbl_edu_knowledge extends db_basic{
...
// 		$this->linkCFG->dbname = 'hx_curriculumn';
	...
	protected function _getTableName($subject_id){
		global $CFG;
		$this->tableName = $CFG['subject'][$subject_id].'_edu_knowledge';
	
	}
	
	//获取所有知识点
	public function get_knowledges($subject_id  ,  $section_id = null){
		...
		$result = $this->select($column,$where,null,'level ASC , sort_id ASC , id ASC');
		return $result;
		
	}
	
	//插入知识点
	public function insert_knowledge($data){
	...
	public function modify_knowledge($subject_id , $knowledge){
		$this->_getTableName($subject_id);
		
		$result = $this->update($knowledge);
		return $result;
	}
	
	public function drag_knowledge($subject_id , $knowledge , $levelDiff , $childs){
		$this->_getTableName($subject_id);
...
SQL;
	...
	public function delete_knowledge($subject_id , $knowledge_id){
		$this->_getTableName($subject_id);
		
		$result = $this->deleteById($knowledge_id);
		return $result;
	}
	
	public function move_up_knowledge($subject_id , $id , $id2){
		$this->_getTableName($subject_id);
		$tableName = $this->tableName;
...
// SQL;
		...
		$sql =<<<SQL
			UPDATE $tableName as t1 
			JOIN $tableName as t2 ON t1.id=$id and t2.id=$id2 
			SET t1.sort_id=t2.sort_id , t2.sort_id=t1.sort_id
SQL;
	...
	public function move_down_knowledge($subject_id , $id , $id2){
		$this->_getTableName($subject_id);
		$tableName = $this->tableName;
	...
	public function copyData($toVersion , $toSubject , $toSection , $fromVersion , $fromSubject , $fromSection){
		...
		foreach($CFG['data']['curriculumn'] as $curriculumn){
			if($curriculumn['id'] == $toVersion){
				$toName = 'curriculumn_source_'.$curriculumn['extends'].'.'.$CFG['subject'][$toSubject].'_edu_knowledge';
			}
			if($curriculumn['id'] == $fromVersion){
				$fromName = 'curriculumn_source_'.$curriculumn['extends'].'.'.$CFG['subject'][$fromSubject].'_edu_knowledge';
			}
		}
		
		$sql = <<<SQL
		
		INSERT INTO $toName (ref_id , name , level , parent_id , subject_id , sort_id , section_id ,  knowledge_id , match_flag , version_id) 
				SELECT id , name , level , parent_id , $toSubject as subject_id , sort_id , $toSection as section_id , knowledge_id , match_flag , $toVersion as version_id FROM $fromName WHERE section_id=$fromSection;
