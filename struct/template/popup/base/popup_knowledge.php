...
dataQueryObject.popup_knowledge = {
...
dataSubmitObject.popup_knowledge = {
	setinData : function(from_version , from_subject , from_section , to_subject , to_section , relation){

		$.ajax({
			'url' : TJDataCenter.get('webroot')+'/manage_system/api/base/knowledge_api.php?action=setin_data',
			'type' : 'POST',
			'dataType' : 'jsonp',
			'data' : {
...
TJEvent.addListener('section_change' , function(e){
	...
	if(version_id !=0 && subject_id != 0 && section_id !=0){
		...
		//选择了学科，学段和版本后，才能获取数据
		dataQueryObject.popup_knowledge.queryTreeData(version_id , subject_id , section_id);
	}
...
$('.setin_data_submit').click(function(){
	...
	if(confirm('确认导入数据吗?')){
		...
		dataSubmitObject.popup_knowledge.setinData(from_version , from_subject , from_section , to_subject , to_section , relation);
