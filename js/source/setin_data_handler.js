...
dataQueryObject = {
	...
	queryTreeData : function(config){
		...
		var curriculumn_version = TJDataCenter.get('curriculumn_version');
	...
	queryQuestionData : function(config){
		...
		var queryData = {
			...
			type : TJDataCenter._urldata['type'],
			cv : TJDataCenter.get('curriculumn_version'),
	...
	queryKnowledgeData : function(){
		...
		var subject = TJDataCenter.get('subject');
		var section = TJDataCenter.get('section');
		var version = TJDataCenter.get('curriculumn_version');
		...
		$.ajax({
			'url' : TJDataCenter.get('webroot')+'/manage_system/api/source/knowledge_api.php?action=tree_data',
			'type' : 'GET',
			'dataType' : 'jsonp',
			'data' : {
				subject : TJDataCenter.get('subject'),
...
dataSubmitObject = {
	...
	setoutExam : function(exam_id){
		...
		if(TJDataCenter.get('curriculumn_version')  == 0){
			alert('请选择需要出库的版本');
			return;
		}
	...
	queryDelay = setTimeout(function(){
		...
		switch(TJDataCenter._urldata['type']){
			...
			case 2 : //如果是同步，则需要依次查询出版社和教材
				if(TJDataCenter.get('subject') != 0 && TJDataCenter.get('grade') != 0){
					dataQueryObject.queryPublisherData()
				}
				break;
				
			case 3 : //如果是专题，需要依次
				if(TJDataCenter.get('subject') != 0 && TJDataCenter.get('section') != 0 && TJDataCenter.get('curriculumn_version') != 0){
					dataQueryObject.queryTreeData()
				}
				break;
		}
...
$(document).delegate('.action_insert' , 'click' , function(){
	...
	var insertElement = TJDataCenter.get('selected_tree_element');
