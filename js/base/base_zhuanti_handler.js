		...
		$.ajax({
			...
			success : function(data){
				if(data){
					...
					e.data.source = 'zhuanti_system_handler.js queryTreeData()';
					TJEvent.dispatch(e);
				}
			},
			error : function(){
...
dataSubmitObject = {
	insertZhuantiData : function(newData){
		$.ajax({
			'url' : TJDataCenter.get('webroot')+'/manage_system/api/base/zhuanti_api.php?action=insert',
			'type' : 'POST',
			'dataType' : 'jsonp',
			'data' : {
				new_data : newData  
	...
	moveUpZhuanti : function(zhuanti_id , target_id , oldname){
		$.ajax({
			'url' : TJDataCenter.get('webroot')+'/manage_system/api/base/zhuanti_api.php?action=move_up',
			'type' : 'POST',
			'dataType' : 'jsonp',
			'data' : {
	...
	moveDownZhuanti : function(zhuanti_id , target_id , oldname){
		$.ajax({
			'url' : TJDataCenter.get('webroot')+'/manage_system/api/base/zhuanti_api.php?action=move_down',
			'type' : 'POST',
			'dataType' : 'jsonp',
			'data' : {
				zhuanti_id : zhuanti_id , 
	...
	modifyZhuanti : function(info , oldname){
		$.ajax({
			'url' : TJDataCenter.get('webroot')+'/manage_system/api/base/zhuanti_api.php?action=modify',
			'type' : 'POST',
			'dataType' : 'jsonp',
			'data' : {
				zhuanti : info , 
			...
			success : function(data){
				if(data){
					var e = new TJEvent.EventObject('modify_zhuanti_success');
					TJEvent.dispatch(e);
				}else{
					var e = new TJEvent.EventObject('modify_zhuanti_failed');
					TJEvent.dispatch(e);
				}
			},
			error : function(){
	...
	dragZhuanti : function(info , levelDiff , childs , oldname){
		$.ajax({
			'url' : TJDataCenter.get('webroot')+'/manage_system/api/base/zhuanti_api.php?action=drag',
			'type' : 'POST',
			'dataType' : 'jsonp',
			'data' : {
				zhuanti : info , 
			...
			success : function(data){
				if(data){
					var e = new TJEvent.EventObject('drag_zhuanti_success');
					TJEvent.dispatch(e);
				}else{
					var e = new TJEvent.EventObject('drag_zhuanti_failed');
					TJEvent.dispatch(e);
				}
			},
			error : function(){
	...
	deleteZhuanti : function(id , oldname){
		$.ajax({
			'url' : TJDataCenter.get('webroot')+'/manage_system/api/base/zhuanti_api.php?action=delete',
			'type' : 'POST',
			'dataType' : 'jsonp',
			'data' : {
				zhuanti_id : id , 
			...
			success : function(data){
				if(data){
					var e = new TJEvent.EventObject('delete_zhuanti_success');
					TJEvent.dispatch(e);
				}else{
					var e = new TJEvent.EventObject('delete_zhuanti_failed');
					TJEvent.dispatch(e);
				}
			},
			error : function(){
	...
	clearData : function(){
		$.ajax({
			'url' : TJDataCenter.get('webroot')+'/manage_system/api/base/zhuanti_api.php?action=clear_data',
			'type' : 'POST',
			'dataType' : 'jsonp',
			'data' : {
				subject : TJDataCenter.get('origin_search_subject'),
...
TJEvent.addListener('edit_name_submit' , function(e){
	...
	dataSubmitObject.modifyZhuanti({ id : id , name : e.data.name} , oldname);

});

TJEvent.addListener('modify_zhuanti_success' , function(){
...
TJEvent.addListener('modify_zhuanti_failed' , function(){
...
//		sort_id : baseElement ? 
//			parseInt(baseElement.closest('li').siblings(':last').prev().find('a').attr('sort_id'))+1 : 
//			($('#treeNav > .child_li > .child_ul > li > a').length > 0 ? parseInt($('#treeNav > .child_li > .child_ul > li > a:last').attr('sort_id'))+1 : 1 ),//在当前层尾部添加
//		version_id : TJDataCenter.get('curriculumn_version')
	};
	
	dataSubmitObject.insertZhuantiData(newData);
...
TJEvent.addListener('insert_zhuanti_failed' , function(){
	alert('插入失败，请稍后再试');
});

TJEvent.addListener('delete_zhuanti_success',function(){
	dataQueryObject.queryTreeData();
});

TJEvent.addListener('delete_zhuanti_failed',function(){
	alert('删除失败，请稍后再试');
});
...
TJEvent.addListener('drop_tree_element' , function(e){
	...
	dataSubmitObject.dragZhuanti(info , levelDiff , childs , oldname);
	
});

TJEvent.addListener('drag_zhuanti_success' , function(e){
	dataQueryObject.queryTreeData();
});
...
//		}
	...
	$('.action_move_up').click(function(){
		if(TJTree.checkFirst()){
			...
			dataSubmitObject.moveUpZhuanti(
	...
	$('.action_move_down').click(function(){
		if(TJTree.checkLast()){
			...
			dataSubmitObject.moveDownZhuanti(
				TJDataCenter.get('origin_tree_selected_tree_element').attr('elementid') , 
	...
	$('.action_delete').click(function(){
		...
		if(!TJTree.checkTail()){
			...
			if (confirm('确认删除“' + TJDataCenter.get('origin_tree_selected_tree_element').attr('title') + '”吗？此操作不可恢复')) {
				...
				dataSubmitObject.deleteZhuanti(id , oldname);
			}
		}
